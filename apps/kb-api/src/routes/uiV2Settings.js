...
"      });",
"      // builds: 6 уровней для metso/koyo, 7 (с object) для universal",
"      var allBuilds = (style === 'universal')",
"        ? ['object','cabinet','station','card','channel','signal','device']",
"        : ['cabinet','station','card','channel','signal','device'];",
"      var selected = new Set(Array.from(wrap.querySelectorAll('#wzBuilds input[type=checkbox]:checked')).map(function (cb) { return cb.getAttribute('data-build'); }));",
"      if (selected.size === 0 && Array.isArray(data.builds)) {",
"        data.builds.forEach(function (b) { selected.add(b); });",
"      }",
...
"      payload.builds = Array.from(wrap.querySelectorAll('#wzBuilds input[type=checkbox]:checked')).map(function (cb) { return cb.getAttribute('data-build'); });",
...
"            setStyle(newStyle);",
"            // Обновить чекбоксы builds под новые данные",
"            var sel = new Set(data.builds || []);",
"            wrap.querySelectorAll('#wzBuilds input[type=checkbox]').forEach(function (cb) {",
"              cb.checked = sel.has(cb.getAttribute('data-build'));",
"            });",
...
"            if (firstSheet && !$('wzSheetFilter').value) {",
"              $('wzSheetFilter').value = '^' + firstSheet.replace(/\\d+/g, '\\\\d+').replace(/[.+?()[\\]{}|^$\\\\]/g, '\\\\$&');",
"            }",
"            if (firstSheet && !$('wzSheetRe').value) {",
"              $('wzSheetRe').value = '^' + firstSheet.replace(/\\d+/g, '\\\\d+').replace(/[.+?()[\\]{}|^$\\\\]/g, '\\\\$&');",
"            }",
"          }",
