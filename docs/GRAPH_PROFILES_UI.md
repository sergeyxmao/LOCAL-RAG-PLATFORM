...
8. **builds** — какие уровни иерархии создавать. Чекбоксы
   рендерятся динамически из таблицы `graph_node_types` через
   `GET /api/v2/graph/node-types` — отображаются русские лейблы и
   emoji-иконки (🗄 Шкаф, ⚡ ПЛК, 🔌 Плата, …), у каждого чекбокса
   есть подсказка `?` с описанием. `value` чекбокса остаётся
   английским кодом (`cabinet`, `station`, …) — именно код пишется
   в YAML профиля и в `graph_nodes.type`. В универсальном стиле
   доступен ещё уровень `object`; в metso/koyo он скрыт.
   Новые кастомные типы, добавленные в подвкладке «Типы узлов»,
   автоматически появляются в этом списке. Подробности —
...
- 2026-05-18: #8.1.e — третья подвкладка «Типы узлов» рядом с
  «Профили парсера» и «Алиасы signal_kind». Чекбоксы `builds`
  wizard'а профилей стали динамическими: лейблы и иконки из
  таблицы `graph_node_types`, подсказки `?` с описанием. AJV
  валидация `builds` ослаблена с enum до паттерна
  `^[a-z][a-z0-9_]*$`. Парсер XLSX продолжает учитывать только
  семь системных кодов; неизвестные коды попадают в
  `graph_report.warnings` под `unknown_node_type`. Подробности —
  `docs/GRAPH_NODE_TYPES.md`.
