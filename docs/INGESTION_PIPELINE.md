...
raw -> parsed -> chunks (Слой 1) -> contextual enrichment (Слой 2) -> embeddings -> qdrant -> metadata

## Качество текстовой базы: Слой 1 (структурный чанкинг) + Слой 2 (обогащение)

### Слой 1 — структурный чанкинг текстовых документов (docx/txt/md)

`chunkTextDocument` (`apps/kb-api/src/services/chunkingService.js`, режим
`structural: true`) режет текст **по смысловым границам**, а не валит всё в
крупные «валуны»:

- сначала по границам абзацев (пустая строка, `\n\n`); одиночный `\n` —
  вторичная граница (записи журнала, элементы списка);
- разнородные блоки **не склеиваются** через `join(" ")` — соседние блоки в
  одном чанке соединяются через `\n`, сохраняя границу;
- внутри длинного блока добиваем по предложениям;
- целевой размер из конфига `chunking.text_max_tokens` (220), перекрытие
  `overlap_sentences` (2); очень короткие соседние блоки объединяются до
  `chunking.text_min_tokens` (60), чтобы не плодить чанки в несколько слов.

Параметры читаются из `config/ingestion.yaml` (не хардкод). **PDF-путь
(постраничные asset-чанки) не затрагивается** — он использует legacy
sentence-стратегию (`structural: false`, `max_tokens` 450).

### Слой 2 — контекстное обогащение чанков (Contextual Retrieval)

Перед индексацией каждый текстовый чанк обогащается облачным LLM: добавляется
контекст «где фрагмент и о чём», теги и краткое описание. В поиск идёт только
**контекст + текст** (`text_with_context`); теги/summary — метаданные. Текст
чанка не переписывается. Вызывается из `ingestionService.enrichChunks` для
текстовых документов; **PDF и CSV/XLSX не покрываются**. Graceful fallback:
при ошибке/без провайдера чанк индексируется без обогащения, импорт не падает.
Полное описание, контракт, настройки и промпты — в
[CONTEXTUAL_ENRICHMENT.md](CONTEXTUAL_ENRICHMENT.md).
