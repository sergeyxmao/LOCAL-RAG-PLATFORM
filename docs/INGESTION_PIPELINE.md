...
> **С #8.1.c.fix-2 чекбокс «Лёгкий режим» удалён из UI Загрузки.**
> UI всегда передаёт `createVisualAssets=true` — превью страниц
> создаётся для каждого документа. Лёгкий режим остаётся **доступным
> через API** (`POST /documents/ingest-folder-async`,
> `POST /jobs/queue` и т.п. принимают `createVisualAssets: false`).
> Это сделано, чтобы UI был проще, а curl-скрипты не сломались.
> Внутренний reindex по-прежнему вызывается с
> `createVisualAssets=true` для полной переиндексации.
