...
- Бэкап делается через `pg_dump` (с флагами `--no-owner --no-privileges`), пайп
  ...
  event loop.
- **Восстановление полностью пересоздаёт схему `public`** — это не наложение
  поверх существующих данных. По шагам:
  1. `pg_terminate_backend` для всех клиентов БД, кроме самой сессии psql
     (включая пул соединений kb-api, иначе `DROP SCHEMA` повиснет).
  2. `DROP SCHEMA IF EXISTS public CASCADE` + `CREATE SCHEMA public` +
     `GRANT ALL ON SCHEMA public TO localrag` + `GRANT ... TO public`.
  3. `gunzip -c | psql -v ON_ERROR_STOP=1 --no-psqlrc` — любая ошибка из
     дампа прерывает восстановление с понятным `Не удалось восстановить
     базу из дампа: psql завершился с кодом …` (никаких «успехов» при
     реальных ошибках).
  4. `ensureRuntimeSchema()` на случай, если бэкап старше текущих миграций
     (добавляет недостающие колонки / таблицы / расширения).
  5. **Контейнер kb-api делает `process.exit(0)` через 800 мс** после
     отправки ответа клиенту. Docker рестартует сервис (есть
     `restart: unless-stopped` в `docker-compose.yml`), и kb-api поднимается
     со свежим пулом соединений. UI настройек поллит `GET /health` раз в
     секунду и автоматически перезагружает страницу, как только сервис
     поднимется (обычно 5–10 секунд).
...
- Восстановление полностью пересоздаёт схему `public` PostgreSQL — текущие
  документы, чаты, настройки, узлы базы знаний будут полностью утеряны без
  отдельного бэкапа.
- После восстановления Qdrant остаётся со старыми векторами. Если в
  восстанавливаемой БД на момент бэкапа были другие документы — векторы
  будут указывать «в никуда», и поиск может ничего не находить или
  путаться. В этом случае запустите «Пересобрать Qdrant» в Настройках →
  Обслуживание (это перестроит векторное хранилище из новых документов).
- После восстановления kb-api автоматически перезапускается. Если этого не
  происходит за 30 секунд — проверьте `docker logs localrag-kb-api`.
...
- 2026-05-16: hotfix #5 — восстановление накатывало дамп поверх существующей
  БД с `ON_ERROR_STOP=0`, из-за чего рапортовало об успехе, не восстановив
  данные (objects already exist → ошибки молча проглатывались). Теперь
  процедура: terminate других подключений → `DROP SCHEMA public CASCADE` →
  `CREATE SCHEMA public` + grants → `psql ON_ERROR_STOP=1` → `process.exit(0)`
  → Docker рестарт. UI поллит `/health` и сам перезагружает страницу.
