...
В новом UI v2: открыть `/ui/v2/settings` → вкладка **«Сервисы»** →
карточка **«Параллелизм индексации»**.

- Поле number `[1..4]` (default 1).
- На слабом CPU держите 1: параллельные ingestion-pipeline'ы
  конкурируют за CPU, RAM и канал к Ollama (embeddings), что
  замедляет общее время.
- На мощной машине можно поставить 2–3.
- Выше 4 нет смысла: Ollama embeddings становится bottleneck'ом
  независимо от CPU.
- Изменение применяется мгновенно. Текущие задачи не прерываются;
  ожидающие подхватят новый лимит.

Важно различать два уровня:

- **Frontend upload concurrency** (`localrag.upload.concurrency`,
  переключатель в Загрузке): сколько PUT /upload одновременно лезет
  на сервер. Защищает сеть.
- **Backend indexing concurrency** (`app_settings.indexing.concurrency`,
  карточка в Настройках): сколько pipeline'ов реально обрабатывает
  файлы. Защищает CPU/RAM/Ollama.

Можно загружать пачкой по 3 (быстрая запись на диск), а
индексировать по 1 (медленный embedding). Это типичная
рекомендация для слабого ноутбука.

Проверить состояние семафора:

```bash
curl -s http://localhost:8787/api/v2/settings/indexing
# → { ok: true, indexing: { concurrency: 1 },
#     semaphore: { max: 1, current: 1, waiting: 4 } }
```

`waiting > 0` означает, что в очереди индексации скопились задачи,
а текущая ещё не закончилась — типично для слабого CPU после
большой пачки загрузок.
