...
7 «встроенных» типов (`object`, `cabinet`, `station`, `card`,
`channel`, `signal`, `device`) создаются автоматически при первом
старте `kb-api` и помечены флагом `is_builtin = TRUE`. Их `code`
неизменяем; русское название, описание, иконку и порядок
сортировки можно править. Удалить их нельзя.
