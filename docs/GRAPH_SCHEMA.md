...
Подключение графа к retrieval/answer реализовано в #8.3 — см.
...
- На слабом ноутбуке: операции — обычные SQL-запросы, нагрузки на
  CPU/диск минимальны.
- При недоступности Postgres все эндпоинты графа возвращают 500. В
  answer-pipeline (#8.3) graph-lookup ловит ошибку и возвращает
  `used:false` — RAG-ответ при этом не падает (см.
  [GRAPH_RETRIEVAL.md](GRAPH_RETRIEVAL.md)).
